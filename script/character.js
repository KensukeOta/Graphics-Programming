  ...
  update() {
    ...
    if (this.isComing === true) {
      ...
      if (window.isKeyDown.key_ArrowRight === true) {
        this.position.x += this.speed; // 右矢印キー
      }
      if (window.isKeyDown.key_ArrowUp === true) {
        this.position.y -= this.speed; // 上矢印キー
      }
      if (window.isKeyDown.key_ArrowDown === true) {
        this.position.y += this.speed; // 下矢印キー
      }
...
class Shot extends Character {
  ...
  update() {
    ...
    this.targetArray.map((v) => {
      if (this.life <= 0 || v.life <= 0) { return; } // 自身か対象のライフが0以下の対象は無視する
      let dist = this.position.distance(v.position); // 自身の位置と対象との距離を測る
      ...
      if (dist <= (this.width + v.width) / 4) {
        ...
        if (v.life <= 0) {
          ...
          if (v instanceof Enemy === true) {
            ...
            // スコアシステムによるが、仮でここでは最大スコアを制限
            gameScore = Math.min(gameScore + score, 99999);
          }
        }
        this.life = 0; // 自身のライフを0にする
      }
    });
  ...
  update() {
    ...
    switch (this.type) {
      ...
      case "large":
        ...
        break;
      // defaultタイプは設定されている進行方向にまっすぐ進むだけの挙動
      case "default":
      default:
...
class Boss extends Character {
   ...
   */
  update() {
    if (this.life <= 0) { return; } // もしボスキャラクターのライフが0以下の場合は何もしない
    ...
    switch (this.mode) {
      ...
      case "escape":
        ...
        break;
      case "floating":
        // 配置後のフレーム数を1000で割ったとき、余りが500未満となる場合と、
        // そうではない場合で、ショットに関する挙動を変化させる
    ...
    this.draw(); // 描画を行う（いまのところ、回転は必要としていないのでそのまま描画）
    ++this.frame; // 自身のフレームをインクリメントする
  }
